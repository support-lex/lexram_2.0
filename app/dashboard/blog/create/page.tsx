...
export default function CreateBlogPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (error || !data) {
        ...
        router.replace("/dashboard/blog/admin");
        return;
      }
  ...
  // ── Auto-generate from a research thread (?source=research) ─────────────
  // The Make Blog button on the research page stashes the bundled session
  // (questions, AI prose, authorities, mermaid diagrams, drafts) into
  // sessionStorage before navigating here. We consume it once on mount,
  // POST to /api/ai/blog, and apply the result to every field. One-shot
  // guard so a re-render or HMR doesn't trigger a second AI call.
  //
  // Persistence on back/forward navigation: after the AI returns, we mirror
  // the generated draft into `BLOG_GENERATED_STORAGE_KEY`. Re-mounting the
  // page with `?source=research` after the source itself has been consumed
  // restores from this key instead of showing a blank editor or re-firing
  // the AI. The key is cleared when the post is saved to Supabase or when
  // the user explicitly starts a new (non-research) draft.
  const sourceParam = searchParams.get("source");
  const generatedFromResearchRef = useRef(false);
  const [generatingFromResearch, setGeneratingFromResearch] = useState(false);

  // Helper that fills every field from a draft object (used by both the
  // freshly-generated AI response and the restore-from-storage path).
  const applyGeneratedDraft = useCallback(
    (data: {
      ...
      if (data.meta_title || data.meta_description) setSeoOpen(true);
    },
    [],
  );

  useEffect(() => {
    if (sourceParam !== "research") return;
    if (generatedFromResearchRef.current) return;
    if (roleLoading || role !== "admin") return;
    if (editId) return; // editing an existing post takes precedence

    // 1) If we've already generated a draft in this tab, restore it instead
    //    of asking the AI again. This is what fixes the "form vanishes on
    //    back/forward" bug — the draft survives across navigations.
    const stored = readGeneratedDraft();
    if (stored) {
      generatedFromResearchRef.current = true;
      applyGeneratedDraft(stored);
      return;
    }

    // 2) Otherwise look for a freshly-stashed research source from the
    //    Make Blog button and run the AI generation.
    const src = consumeBlogSource();
    if (!src) return;
    generatedFromResearchRef.current = true;

    if (src.titleHint && !title) setTitle(src.titleHint);
    ...
    (async () => {
      ...
      try {
        const res = await fetch("/api/ai/blog", {
          ...
          body: JSON.stringify({
            prompt: src.topic || "Blog from LexRam research thread",
            title: src.titleHint,
            mode: "replace",
            researchSource: src,
          }),
        ...
        toast.success("Blog drafted from your research — review and publish");
      } catch (e) {
        toast.error("Couldn't draft from research", { description: (e as Error).message });
      } finally {
        setGeneratingFromResearch(false);
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sourceParam, roleLoading, role, editId, applyGeneratedDraft]);

  // Mirror live edits to the sessionStorage copy so the user's manual
  // refinements (not just the AI's initial output) survive a back/forward
  // navigation. Gated on `generatedFromResearchRef` and `sourceParam` so we
  // don't write this key for unrelated draft sessions.
  useEffect(() => {
    if (sourceParam !== "research") return;
    if (!generatedFromResearchRef.current) return;
  ...
  }, [sourceParam, title, subtitle, category, tags, content, metaTitle, metaDescription]);
  ...
  // Reactive mirror of dirtyRef — drives the Save Draft button's disabled
  // state. We can't disable based on the ref because ref mutations don't
  // re-render; this state changes alongside dirtyRef in the same places.
  ...
  // The dirty effect above fires once on mount with the initial field
  // values, and again after the load effect populates fields from
  // Supabase. Neither is a real user edit, so reset right after load
  // completes. Declared AFTER the dirty effect so its setIsDirty(false)
  // wins in the same batch.
  useEffect(() => {
  ...
  async function doSave(opts?: { silent?: boolean; nextStatus?: BlogStatus }): Promise<BlogPost | null> {
    ...
    try {
      ...
      dirtyRef.current = false;
      setIsDirty(false);
      // Once the post is in Supabase we no longer need the sessionStorage
      // restore copy — Supabase becomes the source of truth on next visit
      // (via the ?edit=<id> path). Leaving it would cause a stale restore.
      clearGeneratedDraft();
      // Swap ?source=research → ?edit=<savedId> in the URL so a back/forward
      // navigation lands in the editId branch (which loads from Supabase).
      // Without this, the URL stays ?source=research, the sessionStorage
      // copy has just been cleared by clearGeneratedDraft above, and the
      // apply effect finds nothing to restore → form vanishes. router.replace
      // (not push) so the back button still returns to research, not to the
      // pre-save state of this page.
      if (sourceParam === "research" && saved.id && !editId) {
        router.replace(`/dashboard/blog/create?edit=${saved.id}`);
      }
      if (!opts?.silent) {
        toast.success(
  ...
  async function handlePublish() {
    ...
    if (saved?.status === "published") router.push(`/dashboard/blog/${saved.slug}`);
  }
  ...
  async function handlePreview() {
    ...
    if (saved) router.push(`/dashboard/blog/${saved.slug}?preview=1`);
  }
  ...
  return (
    // Dashboard layout's <main> wrapper is overflow-hidden (so research-2
    // can manage its own internal scrolling). The editor needs to scroll the
    // whole page, so own the scroll here with `h-full overflow-y-auto`.
    <div className="h-full overflow-y-auto bg-[var(--bg-primary)] pb-32">
      {/* ─────── Sticky header ─────── */}
      <header className="sticky top-0 z-30 bg-[var(--bg-primary)]/85 backdrop-blur border-b border-[var(--border-default)]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {/* Back-to-blog button removed — header is now title + status only. */}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={!isDirty ? "No changes to save" : "Save draft"}
              className="inline-flex items-center gap-1.5 px-3 h-9 rounded-lg text-sm font-medium border border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-primary)] hover:bg-[var(--surface-hover)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-[var(--bg-surface)] transition-colors"
            >
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 sm:pt-10 space-y-6">
        {generatingFromResearch && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[var(--accent)]/30 bg-gradient-to-r from-[var(--accent)]/8 to-purple-500/5 text-sm text-[var(--text-primary)]">
            <Loader2 className="h-4 w-4 animate-spin text-[var(--accent)]" />
            <span>
              Drafting blog from your LexRam research — pulling in authorities, diagrams, and drafts. This usually takes 5–10 seconds.
            </span>
          </div>
        )}
