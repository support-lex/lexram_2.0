...
export default function ProceduralTimeline({ steps, title }: ProceduralTimelineProps) {
  ...
  return (
    <div className="mt-3 rounded-2xl lexram-glass ring-1 ring-[var(--border-default)] shadow-[0_4px_24px_-8px_rgba(10,22,40,0.18)] overflow-hidden">
      {title && (
        <div className="px-6 pt-5 pb-1 text-[10px] font-bold tracking-[0.18em] text-[var(--text-primary)]/60">
          {title.toUpperCase()}
        </div>
      )}
          ...
          {steps.map((step, i) => {
            ...
            const nodeClass =
              variant === 0
                ...
                : variant === 1
                  ? "bg-[var(--accent)] text-[var(--text-primary)] ring-2 ring-[var(--accent)]"
                  : "bg-[var(--surface-hover)] text-[var(--text-primary)] ring-2 ring-[#0A1628]/20 shadow-sm";
            return (
              <li key={`step-${i}`} className="relative pl-14 min-h-[44px]">
                ...
                <div className="pt-1">
                  <div className="text-[15px] font-bold text-[var(--text-primary)] leading-snug">
                    {step.title}
                  </div>
                  {step.detail && (
