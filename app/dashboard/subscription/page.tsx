...
const FAQS = [
  { question: "Do credits expire?", answer: "No. Credits you purchase never expire and carry forward indefinitely." },
  { question: "How are credits consumed?", answer: "Each research query costs ~100 credits. Drafting costs ~250 credits. Simple lookups cost ~20 credits." },
  { question: "Can I share credits with my team?", answer: "Yes, with the Chamber Bulk plan. Up to 5 users can share a single credit pool." },
  { question: "What payment methods do you accept?", answer: "UPI, net banking, credit/debit cards, and bank transfer for Chamber plans." },
];

const PAGE_URL = `${SITE_URL}/dashboard/subscription`;
          ...
          <div className="mt-12">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {FAQS.map(({ question, answer }) => (
                <div key={question} className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-4">
                  <div className="text-sm font-semibold text-[var(--text-primary)] mb-1">{question}</div>
                  <div className="text-xs text-[var(--text-secondary)] leading-relaxed">{answer}</div>
                </div>
              ))}
            </div>
          </div>
