    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[var(--bg-sidebar)]/90 backdrop-blur-xl border-b border-[var(--border-default)] py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <Link href="/acts" className="font-sans text-sm font-normal tracking-wide text-[var(--text-on-sidebar)]/80 hover:text-[var(--text-on-sidebar)] transition-colors">Resources</Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-[var(--bg-sidebar)] border-b border-[var(--border-default)] px-4 pt-4 pb-6 space-y-2 absolute top-full left-0 w-full">
          ...
          <Link href="/acts" className="block px-4 py-3 font-sans text-base font-normal text-[var(--text-on-sidebar)]/80 hover:bg-[var(--bg-sidebar-hover)] rounded-xl" onClick={() => setIsMobileMenuOpen(false)}>Resources</Link>
