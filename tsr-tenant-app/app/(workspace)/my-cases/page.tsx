...
import { getDocumentViewUrl, type TsrCaseSummary, type TsrDocument } from "@/lib/tsr-api";
import { listCases } from "@/lib/tsr-data";
...
const STATUS_PILL: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  uploaded: { bg: "#E0F2FE", text: "#075985", dot: "#0284C7" },
...
function DocumentRow({ caseId, doc }: { caseId: string; doc: TsrDocument }) {
  ...
  return (
    <tr className="border-t border-maroon/8 hover:bg-cream-warm/30 transition-colors">
      ...
      <td className="px-4 py-3 text-sm text-ink/70">{doc.page_count != null ? doc.page_count : <span className="text-ink/40">—</span>}</td>
    ...
    <>
      <tr className="border-t border-maroon/10 hover:bg-cream-warm/20 transition-colors">
        <td className="px-4 py-3.5">
          <button onClick={() => setOpen((o) => !o)} className="flex items-start gap-2 text-left group" aria-expanded={open}>
        ...
        <td className="px-4 py-3.5 text-sm text-ink/75">{row.token_usage ? fmtNum(row.token_usage.total_tokens) : <span className="text-ink/40">—</span>}</td>
        <td className="px-4 py-3.5 text-sm text-ink/60">{fmtDate(row.created_at)}</td>
        <td className="px-4 py-3.5 text-right">
          {hasReport ? (
            <Link href={`/${row.id}`} className="inline-flex items-center gap-1.5 text-xs font-medium text-maroon hover:text-rust transition-colors">
              <Download className="w-3.5 h-3.5" /> Open
            </Link>
          ) : (
            <Link href={`/${row.id}`} className="text-xs text-ink/40 hover:text-maroon">Open file</Link>
          )}
        </td>
      </tr>
      {open && (
            ...
            {row.documents.length === 0 ? (
              ...
              <div className="overflow-x-auto rounded-lg border border-maroon/8 bg-cream">
                <table className="w-full text-left">
                  ...
                  <tbody>
                    {row.documents.map((d) => <DocumentRow key={d.id} caseId={row.id} doc={d} />)}
                  </tbody>
                </table>
              </div>
            )}
...
export default function MyCasesPage() {
  ...
  const fetchData = () => {
    ...
    setLoading(true);
    setError(null);
    listCases(session.userId)
      .then(setCases)
      .catch((err) => setError(err instanceof Error ? err.message : String(err)))
      .finally(() => setLoading(false));
  };

  useEffect(() => { if (session.userId) fetchData(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [session.userId]);
  ...
  return (
    <div className="min-h-full px-4 sm:px-6 py-8 sm:py-10 max-w-6xl mx-auto w-full bg-cream">
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-maroon/10 text-maroon text-[10px] font-bold tracking-[0.18em] uppercase mb-3">
            <Sparkles className="w-3 h-3 text-rust" /> My Reports
          </div>
          <h1 className="font-display text-3xl sm:text-4xl font-bold leading-tight text-maroon">My Reports</h1>
          <p className="text-sm sm:text-base text-ink/65 mt-2 max-w-2xl">
            Every client file you&apos;ve created, with its documents and scrutiny status. Click a row to expand the document list.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3 p-4 mb-6 rounded-xl bg-red-50 border border-red-200">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-red-800/90 mt-0.5 leading-relaxed">
              {/PGRST|schema|exposed|not exist|permission/i.test(error)
                ? `The "${process.env.NEXT_PUBLIC_ORG_SLUG}" schema may not be exposed to the API yet. Add it under Supabase → Settings → API → Exposed schemas, then reload.`
                : error}
            </p>
          </div>
      ...
      {loading && <div className="flex items-center justify-center py-20"><Loader2 className="w-6 h-6 animate-spin text-maroon" /></div>}
      ...
      {!loading && cases && cases.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center py-20 px-6 rounded-2xl border border-maroon/10 bg-cream-soft/60">
          ...
          <p className="text-sm text-ink/60 max-w-sm">Open the sidebar and click <strong className="text-rust">New Report</strong> to grant a Title Scrutiny Report.</p>
        </div>
      )}
        ...
        <div className="rounded-2xl border border-maroon/12 bg-cream-soft overflow-hidden shadow-soft">
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>{cases.map((c) => <CaseRow key={c.id} row={c} />)}</tbody>
            </table>
          </div>
        </div>
      )}
