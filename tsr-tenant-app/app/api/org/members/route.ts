// Org-admin invite endpoint (ported from lexram).
...
export async function POST(req: NextRequest) {
  ...
  await sb.from("organization_members").update({ status: "suspended" })
    .eq("user_id", user_id).neq("status", "suspended").neq("org_id", org_id);

  const { data: member, error } = await sb
    .from("organization_members")
    .upsert({
      org_id, user_id, role, status: invite_sent ? "invited" : "active",
  ...
  return NextResponse.json({
    ...
    joined_at: member.joined_at, last_active_at: member.last_active_at, case_count: 0,
  });
}
