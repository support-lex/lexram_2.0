// POST /api/tsr/payments/[id]/confirm — flip a payment row to success.
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: payment, error: getErr } = await sb.from("tsr_payments").select("*").eq("id", id).maybeSingle();
  ...
  if (payment.status === "success") return NextResponse.json(payment);

  const { data: updated, error: updErr } = await sb
    .from("tsr_payments")
    .update({ status: "success", paid_at: new Date().toISOString(), cashfree_payment_id: body.cashfree_payment_id ?? null })
